...
const deleteFile = (identifier: string) => {
  axios.delete('/api/file/' + encodeURI(identifier)).then(() => {
...
watch(
  () => route.query.file,
  () => (selectedFile.value = String(route.query.file))
)
</script>
...
<template>
  ...
  <split-button-group ref="toggleRef" alignment="right">
    <template v-slot:btn-left>
      <router-link
        :to="'/log?file=' + encodeURI(file.identifier)"
      ...
      >
        ...
        <span class="d-block file-size text-secondary text-nowrap overflow-hidden">{{
          file.size_formatted
        }}</span>
      </router-link>
    </template>
    ...
    <template v-slot:dropdown>
      <li>
        <a
          class="dropdown-item"
          :href="baseUri + 'api/file/' + encodeURI(file.identifier)"
      ...
      <li>
        <a
          class="dropdown-item"
          href="javascript:"
          @click="deleteFile(file.identifier)"
