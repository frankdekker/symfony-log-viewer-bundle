...
const navigate = () => {
  const fileOffset =
    offset.value > 0 && logRecordStore.records.paginator?.direction !== sort.value
      ? 0
      : offset.value
  router.push({
    query: filter({
      ...
      levels: nullify(
        levels.value.selected.join(','),
        Object.keys(logRecordStore.records.levels.choices).join(',')
      ),
      channels: nullify(
        channels.value.selected.join(','),
        Object.keys(logRecordStore.records.channels.choices).join(',')
      ),
...
<template>
  <div
    class="slv-content h-100 overflow-hidden slv-loadable"
    v-bind:class="{ 'slv-loading': logRecordStore.loading }"
  >
    ...
    <footer class="pt-1 pb-1 d-flex">
      ...
      <button
        ...
        @click="
          offset = logRecordStore.records.paginator?.offset ?? 0
          navigate()
        "
